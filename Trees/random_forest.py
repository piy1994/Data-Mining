...
class decision_tree_rand:
    ...
    def train(self,train_dataset):
        self.num_features = int((len(train_dataset[0])-1)**0.5)
...
#we convert to list the array numpy
    ...
    def train_rand(self,dataset):
        lst_dt = []
        sz = int(dataset.shape[0])
        for i in range(self.num_trees):
            print('on {} tree'.format(i+1))
            d_t = decision_tree_rand(max_depth=self.max_depth,min_size=self.min_size)
...
def random_fnc(XY_train,XY_test,max_depth,min_size,num_trees):
    rand_t= random_forest(max_depth=max_depth,min_size=min_size,num_trees=num_trees)
    rand_t.train_rand(XY_train)
    pred_y_arr = np.array(rand_t.test_rand(XY_test))
