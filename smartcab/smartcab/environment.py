...
class TrafficLight(object):
    ...
    valid_states = [True, False]  # True = North South open; False = East West open
...
class Environment(object):
    ...
    def __init__(self, verbose=False, num_dummies=100, grid_size = (8, 6)):
        ...
        self.bounds = (1, 2, self.grid_size[0], self.grid_size[1] + 1)  #limits for the environment whole
        self.block_size = 100
        self.hang = 0.6 # road distance from the environment boundary
        self.intersections = OrderedDict()
        # gives the points where traffic light is present
        self.roads = []
        ...
        for a in self.intersections:
            for b in self.intersections:
                ...
                if (abs(a[0] - b[0]) + abs(a[1] - b[1])) == 1:  # L1 distance = 1
                    self.roads.append((a, b)) ## make a road 
        ...
        for i in xrange(self.num_dummies):
            ...
            # these are the dummy agents. default = 100
    ...
    def create_agent(self, agent_class, *args, **kwargs):
        ...
        # self.agent was a orderedDict() and this is a dict itelf !
        # are they by default in north/south ?
        return agent
    ...
    def reset(self, testing=False):
        ...
        # Reset status text
        # this is the thing that is visisble on the cmd !
        self.step_data = {}
    ...
    def step(self):
        ...
        # Update agent_statess, primary first
        # first check in case it is defined
        ...
        if self.primary_agent is not None:
            ...
            # time left is less than than the hard_time_limit !!
    ...
    def sense(self, agent):
        ...
        # for north-south heading[1]!=0 or east west heading[0]!=0
        ...
        for other_agent, other_state in self.agent_states.iteritems():
            # We need for environment so not the same agent, if not same location then its not the envi
            # - ronment and if same location and same heading continue
    ...
    def act(self, agent, action):
        ...
        location = state['location']
        heading = state['heading']
        # True = North South open; False = East West open
        light = 'green' if (self.intersections[location].state and heading[1] != 0) or ((not self.intersections[location].state) and heading[0] != 0) else 'red'
...
class Agent(object):
    ...
    """ Here get_next_way_point is not defined as such. In the learningAgent it will be
        defined more throughly using planner modeule"""
